...
!  Intensities
      ...
      REAL(KIND=dp), DIMENSION(:,:,:), ALLOCATABLE   :: INTENSITY_TOA_SAVE
      REAL(KIND=dp), DIMENSION(:,:,:), ALLOCATABLE   :: INTENSITY_BOA_SAVE

!  Flux output
!     ! @@ Rob Spurr, 05 November 2013, Version 2.3 --> Flux Output

      REAL(kind=dp), DIMENSION(:,:,:,:), ALLOCATABLE :: FLUXES_TOA_SAVE
      REAL(kind=dp), DIMENSION(:,:,:,:), ALLOCATABLE :: FLUXES_BOA_SAVE

!  Output solutions at ALL levels
!     ! @@ Rob Spurr, 17 July 2013, Version 2.2 --> Optional Output at ALL LEVELS

      REAL(kind=dp), DIMENSION(:,:,:,:), ALLOCATABLE :: RADLEVEL_UP_SAVE
      REAL(kind=dp), DIMENSION(:,:,:,:), ALLOCATABLE :: RADLEVEL_DN_SAVE
...
!     Allocate output memory
      ...
      ALLOCATE ( INTENSITY_TOA_SAVE ( MAX_GEOMETRIES, MAXTHREADS, OMP_MAXTHREADS ), &
                 INTENSITY_BOA_SAVE ( MAX_GEOMETRIES, MAXTHREADS, OMP_MAXTHREADS ), &
                 FLUXES_TOA_SAVE    ( MAXBEAMS, 2, MAXTHREADS, OMP_MAXTHREADS ), &
                 FLUXES_BOA_SAVE    ( MAXBEAMS, 2, MAXTHREADS, OMP_MAXTHREADS ), &
                 RADLEVEL_UP_SAVE   ( MAX_GEOMETRIES, 0:MAXLAYERS, MAXTHREADS, OMP_MAXTHREADS ), &
                 RADLEVEL_DN_SAVE   ( MAX_GEOMETRIES, 0:MAXLAYERS, MAXTHREADS, OMP_MAXTHREADS ) )
...
!$OMP     NUMWVN, NTHREADS, NLAYERS, NTOTAL, STREAM_VALUE, &
!$OMP     N_USER_ANGLES, N_USER_OBSGEOMS, &
!$OMP     BEAM_SZAS, USER_ANGLES, USER_RELAZMS, USER_OBSGEOMS, &
!$OMP     FLUX_FACTOR, &
!$OMP     EARTH_RADIUS, HEIGHT_GRID, AIRTEMPS, SURFTEMP, WINDSPEED_SAVE, &
!$OMP     MOLEXT, MOLOMG, RAYMOMS, N6, GAER, WAER, TAER, &
!$OMP     ALBEDO_SAVE, NSTREAMS_BRDF, &
!$OMP     INTENSITY_TOA_SAVE, INTENSITY_BOA_SAVE, &
!$OMP     FLUXES_TOA_SAVE, FLUXES_BOA_SAVE, &
!$OMP     RADLEVEL_UP_SAVE, RADLEVEL_DN_SAVE, N_GEOMETRIES) &
...
!  Save the last results produced by the OMP thread
      ...
      IF (wvn == (TID+1)*numwvn/OMP_NTHREADS) THEN
        INTENSITY_TOA_SAVE ( 1:N_GEOMETRIES, T, TID+1 ) = &
             INTENSITY_TOA ( 1:N_GEOMETRIES )
        INTENSITY_BOA_SAVE ( 1:N_GEOMETRIES, T, TID+1 ) = &
             INTENSITY_BOA ( 1:N_GEOMETRIES )
        FLUXES_TOA_SAVE ( 1:NBEAMS, :, T, TID+1 ) = &
             FLUXES_TOA ( 1:NBEAMS, : )
        FLUXES_BOA_SAVE ( 1:NBEAMS, :, T, TID+1 ) = &
             FLUXES_BOA ( 1:NBEAMS, : )

        IF ( DO_2S_LEVELOUT ) THEN
          RADLEVEL_UP_SAVE ( 1:N_GEOMETRIES, 0:NLAYERS, T, TID+1 ) = &
               RADLEVEL_UP ( 1:N_GEOMETRIES, 0:NLAYERS )
          RADLEVEL_DN_SAVE ( 1:N_GEOMETRIES, 0:NLAYERS, T, TID+1 ) = &
...
!  Begin file write loop
      ...
      do ompt=1,OMP_NTHREADS

!  Prepare basic output file

      write(ch1,'(i1)')   omp_nthreads
      write(ch2,'(i5.5)') numwvn 
      write(ch3,'(i5.5)') ompt*numwvn/omp_nthreads
      tail = '_nt' // ch1 // '_nwn' // ch2 // '_wvn' // ch3

      if ( do_user_obsgeoms ) &
        OPEN(36,file = 'results_2s_tester.all_OBSGEOM' // trim(tail), &
             status = 'unknown')
      if ( .not. do_user_obsgeoms ) &
        OPEN(36,file = 'results_2s_tester.all_LATTICE' // trim(tail), &
             status = 'unknown')
...
!  Intensity output
      ...
      do g = 1, n_geometries
         write(36,366)g,'Upwelling @ ',0.0d0, (INTENSITY_TOA_SAVE(G,T,ompt),T=1,NTHREADS)
         write(36,366)g,'Dnwelling @ ',23.0d0,(INTENSITY_BOA_SAVE(G,T,ompt),T=1,NTHREADS)
      enddo
...
!  Mean output
      ...
      do v = 1, nbeams
         write(36,367)v,'Upwelling @ ',0.0d0, &
            (FLUXES_TOA_SAVE(v,1,t,ompt),t=1,2),(FLUXES_TOA_SAVE(v,2,t,ompt),t=1,2)
         write(36,367)v,'Dnwelling @ ',23.0d0, &
            (FLUXES_BOA_SAVE(v,1,t,ompt),t=1,2),(FLUXES_BOA_SAVE(v,2,t,ompt),t=1,2)
      enddo
...
!  Prepare level output file if selected
      ...
      if ( DO_2S_LEVELOUT ) Then
         if ( do_user_obsgeoms ) &
           OPEN(36,file = 'results_2s_tester.LEVOUT_OBSGEOM' // trim(tail), &
                status = 'unknown')
         if ( .not. do_user_obsgeoms ) &
           OPEN(36,file = 'results_2s_tester.LEVOUT_LATTICE' // trim(tail), &
                status = 'unknown')
         ...
         do g = 1, n_geometries
            do n = 0, nlayers
               write(36,368)g,n,'Upwelling @ ',DBLE(N), &
                  (RADLEVEL_UP_SAVE(G,N,T,ompt),T=1,NTHREADS)
            enddo
            do n = 0, nlayers
               write(36,368)g,n,'Dnwelling @ ',DBLE(N), &
                  (RADLEVEL_DN_SAVE(G,N,T,ompt),T=1,NTHREADS)
            enddo
            write(36,*)
         enddo
...
!  Mean output
      ...
      do v = 1, nbeams
         write(36,369)v,'Upwelling @ ',0.0d0 , (FLUXES_TOA_SAVE(v,1,t,ompt),t=1,NTHREADS)
         write(36,369)v,'Dnwelling @ ',23.0d0, (FLUXES_BOA_SAVE(v,1,t,ompt),t=1,NTHREADS)
      enddo
      ...
      do v = 1, nbeams
         write(36,369)v,'Upwelling @ ',0.0d0 , (FLUXES_TOA_SAVE(v,2,t,ompt),t=1,NTHREADS)
         write(36,369)v,'Dnwelling @ ',23.0d0, (FLUXES_BOA_SAVE(v,2,t,ompt),t=1,NTHREADS)
      enddo
369   format(i5,T11,a,f6.2,1x,6(1x,1pe16.8,1x))
      ...
      if ( DO_2S_LEVELOUT ) Then
         if ( do_pentadiag_inverse ) then
            if ( do_user_obsgeoms)      OPEN(36,file = 'results_2s_tester_PDI.LEVOUT_OBSGEOM' // trim(tail), &
                                              status = 'unknown')
            if ( .not.do_user_obsgeoms) OPEN(36,file = 'results_2s_tester_PDI.LEVOUT_LATTICE' // trim(tail), &
                                              status = 'unknown')
         else
            if ( do_user_obsgeoms)      OPEN(36,file = 'results_2s_tester.LEVOUT_OBSGEOM' // trim(tail), &
                                              status = 'unknown')
            if ( .not.do_user_obsgeoms) OPEN(36,file = 'results_2s_tester.LEVOUT_LATTICE' // trim(tail), &
                                              status = 'unknown')
         endif
         ...
         do g = 1, n_geometries
            do n = 0, nlayers
               write(36,370)g,n,'Upwelling @',DBLE(N), (RADLEVEL_UP_SAVE(G,N,T,ompt),T=1,NTHREADS)
            enddo
            do n = 0, nlayers
               write(36,370)g,n,'Dnwelling @',DBLE(N), (RADLEVEL_DN_SAVE(G,N,T,ompt),T=1,NTHREADS)
            enddo
            write(36,*)
         enddo
...
END PROGRAM test_2s_ONLY_2p4_OMP
