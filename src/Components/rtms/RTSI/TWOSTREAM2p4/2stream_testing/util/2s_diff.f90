...
!INTRINSIC SUBPROGRAMS USED BY TWO_S_DIFF***************************************
!     TRIM,ADJUSTL,LEN
!*******************************************************************************

!EXTERNAL SUBPROGRAMS USED BY TWO_S_DIFF****************************************
!     GETARG
!*******************************************************************************
...
!STANDARD UNIX DIFF UTILITY AND THEN SEND THE RESULTS TO THE OUTPUT FILE
      ...
      DO I=1,FileSize(1)
        ...
        IF (NumLineDiffs > 0) THEN
          DO J=1,StrLen
!mick fix 2/3/2015 - adjust valid result discriminator
...
!DEALLOCATE SOME ARRAYS
      ...
      IF (TEST) THEN
        WRITE(*,*) 'FINISHED WITH CURRENT CALL TO TWO_S_DIFF'
        READ(*,*)
      END IF

!END PROGRAM
      !WRITE(*,*)
      !WRITE(*,*) 'TWO_S_DIFF DONE'
      !WRITE(*,*) '*****************************************************'
      !WRITE(*,*) '*****************************************************'

      END PROGRAM TWO_S_DIFF
