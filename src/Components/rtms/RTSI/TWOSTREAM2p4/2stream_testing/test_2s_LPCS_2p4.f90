...
!  Radiance & Radiance Jacobian output - TOA and BOA
      ...
      REAL(kind=dp) :: INTENSITY_TOA_BAS ( MAX_GEOMETRIES, MAXTHREADS )
      REAL(kind=dp) :: INTENSITY_BOA_BAS ( MAX_GEOMETRIES, MAXTHREADS )

      REAL(kind=dp) :: PROFILEWF_TOA_BAS ( MAX_GEOMETRIES, MAXLAYERS, MAX_ATMOSWFS, MAXTHREADS )
      REAL(kind=dp) :: PROFILEWF_BOA_BAS ( MAX_GEOMETRIES, MAXLAYERS, MAX_ATMOSWFS, MAXTHREADS )

      REAL(kind=dp) :: COLUMNWF_TOA_BAS  ( MAX_GEOMETRIES, MAX_ATMOSWFS, MAXTHREADS )
      REAL(kind=dp) :: COLUMNWF_BOA_BAS  ( MAX_GEOMETRIES, MAX_ATMOSWFS, MAXTHREADS )

      REAL(kind=dp) :: SURFACEWF_TOA_BAS ( MAX_GEOMETRIES, MAX_SURFACEWFS, MAXTHREADS )
      REAL(kind=dp) :: SURFACEWF_BOA_BAS ( MAX_GEOMETRIES, MAX_SURFACEWFS, MAXTHREADS )
...
!     ! @@ Rob Spurr, 28 July 2013, Version 2.2 --> Optional Output at ALL LEVELS
      ...
      REAL(kind=dp) :: RADLEVEL_UP_BAS (MAX_GEOMETRIES,0:MAXLAYERS,MAXTHREADS)
      REAL(kind=dp) :: RADLEVEL_DN_BAS (MAX_GEOMETRIES,0:MAXLAYERS,MAXTHREADS)

      REAL(kind=dp) :: PROFJACLEVEL_UP_BAS (MAX_GEOMETRIES,0:MAXLAYERS,MAXLAYERS,MAX_ATMOSWFS,MAXTHREADS)
      REAL(kind=dp) :: PROFJACLEVEL_DN_BAS (MAX_GEOMETRIES,0:MAXLAYERS,MAXLAYERS,MAX_ATMOSWFS,MAXTHREADS)

      REAL(kind=dp) :: COLJACLEVEL_UP_BAS  (MAX_GEOMETRIES,0:MAXLAYERS,MAX_ATMOSWFS,MAXTHREADS)
      REAL(kind=dp) :: COLJACLEVEL_DN_BAS  (MAX_GEOMETRIES,0:MAXLAYERS,MAX_ATMOSWFS,MAXTHREADS)

      REAL(kind=dp) :: SURFJACLEVEL_UP_BAS (MAX_GEOMETRIES,0:MAXLAYERS,MAX_SURFACEWFS,MAXTHREADS)
      REAL(kind=dp) :: SURFJACLEVEL_DN_BAS (MAX_GEOMETRIES,0:MAXLAYERS,MAX_SURFACEWFS,MAXTHREADS)

!  Flux & Flux Jacobian output - TOA and BOA
!     ! @@ Rob Spurr, 05 November 2013, Version 2.3 --> Flux Output

      REAL(kind=dp) :: FLUXES_TOA_BAS (MAXBEAMS,2,MAXTHREADS)
      REAL(kind=dp) :: FLUXES_BOA_BAS (MAXBEAMS,2,MAXTHREADS)

      REAL(kind=dp) :: PROFJACFLUXES_TOA_BAS (MAXBEAMS,2,MAXLAYERS,MAX_ATMOSWFS,MAXTHREADS)
      REAL(kind=dp) :: PROFJACFLUXES_BOA_BAS (MAXBEAMS,2,MAXLAYERS,MAX_ATMOSWFS,MAXTHREADS)

      REAL(kind=dp) :: COLJACFLUXES_TOA_BAS  (MAXBEAMS,2,MAX_ATMOSWFS,MAXTHREADS)
      REAL(kind=dp) :: COLJACFLUXES_BOA_BAS  (MAXBEAMS,2,MAX_ATMOSWFS,MAXTHREADS)

      REAL(kind=dp) :: SURFJACFLUXES_TOA_BAS (MAXBEAMS,2,MAX_SURFACEWFS,MAXTHREADS)
      REAL(kind=dp) :: SURFJACFLUXES_BOA_BAS (MAXBEAMS,2,MAX_SURFACEWFS,MAXTHREADS)
...
!  Radiance & Radiance Jacobian output - TOA and BOA
      ...
      REAL(kind=dp) :: INTENSITY_TOA_PROF_PT ( MAX_GEOMETRIES, MAXLAYERS, MAX_ATMOSWFS, MAXTHREADS )
      REAL(kind=dp) :: INTENSITY_BOA_PROF_PT ( MAX_GEOMETRIES, MAXLAYERS, MAX_ATMOSWFS, MAXTHREADS )

      REAL(kind=dp) :: INTENSITY_TOA_COL_PT  ( MAX_GEOMETRIES, MAX_ATMOSWFS, MAXTHREADS )
      REAL(kind=dp) :: INTENSITY_BOA_COL_PT  ( MAX_GEOMETRIES, MAX_ATMOSWFS, MAXTHREADS )

      REAL(kind=dp) :: INTENSITY_TOA_SURF_PT ( MAX_GEOMETRIES, MAX_SURFACEWFS, MAXTHREADS )
      REAL(kind=dp) :: INTENSITY_BOA_SURF_PT ( MAX_GEOMETRIES, MAX_SURFACEWFS, MAXTHREADS )

!  Radiance & Radiance Jacobian output - all levels
!     ! @@ Rob Spurr, 28 July 2013, Version 2.2 --> Optional Output at ALL LEVELS

      REAL(kind=dp) :: RADLEVEL_UP_PROF_PT (MAX_GEOMETRIES,0:MAXLAYERS, MAXLAYERS, MAX_ATMOSWFS, MAXTHREADS)
      REAL(kind=dp) :: RADLEVEL_DN_PROF_PT (MAX_GEOMETRIES,0:MAXLAYERS, MAXLAYERS, MAX_ATMOSWFS, MAXTHREADS)

      REAL(kind=dp) :: RADLEVEL_UP_COL_PT  (MAX_GEOMETRIES,0:MAXLAYERS, MAX_ATMOSWFS, MAXTHREADS)
      REAL(kind=dp) :: RADLEVEL_DN_COL_PT  (MAX_GEOMETRIES,0:MAXLAYERS, MAX_ATMOSWFS, MAXTHREADS)

      REAL(kind=dp) :: RADLEVEL_UP_SURF_PT (MAX_GEOMETRIES,0:MAXLAYERS, MAX_SURFACEWFS, MAXTHREADS)
      REAL(kind=dp) :: RADLEVEL_DN_SURF_PT (MAX_GEOMETRIES,0:MAXLAYERS, MAX_SURFACEWFS, MAXTHREADS)

!  Flux & Flux Jacobian output - TOA and BOA
!     ! @@ Rob Spurr, 05 November 2013, Version 2.3 --> Flux Output

      REAL(kind=dp) :: FLUXES_TOA_PROF_PT (MAXBEAMS,2,MAXLAYERS,MAX_ATMOSWFS,MAXTHREADS)
      REAL(kind=dp) :: FLUXES_BOA_PROF_PT (MAXBEAMS,2,MAXLAYERS,MAX_ATMOSWFS,MAXTHREADS)

      REAL(kind=dp) :: FLUXES_TOA_COL_PT  (MAXBEAMS,2,MAX_ATMOSWFS,MAXTHREADS)
      REAL(kind=dp) :: FLUXES_BOA_COL_PT  (MAXBEAMS,2,MAX_ATMOSWFS,MAXTHREADS)

      REAL(kind=dp) :: FLUXES_TOA_SURF_PT (MAXBEAMS,2,MAX_SURFACEWFS,MAXTHREADS)
      REAL(kind=dp) :: FLUXES_BOA_SURF_PT (MAXBEAMS,2,MAX_SURFACEWFS,MAXTHREADS)
...
!  (Note: surface-leaving not active yet)
      ...
      !DO_SLEAVE_WFS         = .true.
      !N_SLEAVE_WFS          = 1
      !LSSL_SLTERM_ISOTROPIC = 1.0D-1
      !LSSL_SLTERM_F_0       = 1.0D-1
...
!  Open output files
      ...
      if ( do_user_obsgeoms ) then
         if ( do_pentadiag_inverse ) then
           OPEN(36,file = 'results_2s_LPCS_tester_PDI.TOAUP_OBSGEOM', status = 'unknown')
           OPEN(37,file = 'results_2s_LPCS_tester_PDI.BOADN_OBSGEOM', status = 'unknown')
         else
           OPEN(36,file = 'results_2s_LPCS_tester.TOAUP_OBSGEOM', status = 'unknown')
           OPEN(37,file = 'results_2s_LPCS_tester.BOADN_OBSGEOM', status = 'unknown')
         endif
      else
         if ( do_pentadiag_inverse ) then
           OPEN(36,file = 'results_2s_LPCS_tester_PDI.TOAUP_LATTICE', status = 'unknown')
           OPEN(37,file = 'results_2s_LPCS_tester_PDI.BOADN_LATTICE', status = 'unknown')
         else
           OPEN(36,file = 'results_2s_LPCS_tester.TOAUP_LATTICE', status = 'unknown')
           OPEN(37,file = 'results_2s_LPCS_tester.BOADN_LATTICE', status = 'unknown')
         endif
      endif
      ...
      if ( do_2S_LEVELOUT ) then
         if ( do_user_obsgeoms ) then
            if ( do_pentadiag_inverse ) then
               OPEN(336,file = 'results_2s_LPCS_tester_PDI.LEVOUT_UP_OBSGEOM', status = 'unknown')
               OPEN(337,file = 'results_2s_LPCS_tester_PDI.LEVOUT_DN_OBSGEOM', status = 'unknown')
            else
               OPEN(336,file = 'results_2s_LPCS_tester.LEVOUT_UP_OBSGEOM', status = 'unknown')
               OPEN(337,file = 'results_2s_LPCS_tester.LEVOUT_DN_OBSGEOM', status = 'unknown')
            endif
         else
            if ( do_pentadiag_inverse ) then
               OPEN(336,file = 'results_2s_LPCS_tester_PDI.LEVOUT_UP_LATTICE', status = 'unknown')
               OPEN(337,file = 'results_2s_LPCS_tester_PDI.LEVOUT_DN_LATTICE', status = 'unknown')
            else
               OPEN(336,file = 'results_2s_LPCS_tester.LEVOUT_UP_LATTICE', status = 'unknown')
               OPEN(337,file = 'results_2s_LPCS_tester.LEVOUT_DN_LATTICE', status = 'unknown')
            endif
         endif
      endif
...
!  Initialize saved output for Fluxes and associated Jacobians
      ...
      write(*,*)'Profile WF baseline: Intensities, 2 Profile WFs + 1 Surface WF'
...
!  Save Baseline results from this call

        INTENSITY_TOA_BAS(1:N_GEOMETRIES,T) = INTENSITY_TOA(1:N_GEOMETRIES)
        INTENSITY_BOA_BAS(1:N_GEOMETRIES,T) = INTENSITY_BOA(1:N_GEOMETRIES)
        if ( do_2s_LEVELOUT ) THEN
           RADLEVEL_UP_BAS(1:N_GEOMETRIES,0:NLAYERS,T) = RADLEVEL_UP(1:N_GEOMETRIES,0:NLAYERS)
           RADLEVEL_DN_BAS(1:N_GEOMETRIES,0:NLAYERS,T) = RADLEVEL_DN(1:N_GEOMETRIES,0:NLAYERS)
        endif
        FLUXES_TOA_BAS(1:NBEAMS,:,T) = FLUXES_TOA(1:NBEAMS,:)
        FLUXES_BOA_BAS(1:NBEAMS,:,T) = FLUXES_BOA(1:NBEAMS,:)

        DO N = 1, NLAYERS
           DO Q = 1, LAYER_VARY_NUMBER(N)
              PROFILEWF_TOA_BAS(1:N_GEOMETRIES,N,Q,T) = PROFILEWF_TOA(1:N_GEOMETRIES,N,Q)
              PROFILEWF_BOA_BAS(1:N_GEOMETRIES,N,Q,T) = PROFILEWF_BOA(1:N_GEOMETRIES,N,Q)
              if ( do_2s_LEVELOUT ) THEN
                 PROFJACLEVEL_UP_BAS(1:N_GEOMETRIES,0:NLAYERS,N,Q,T) = PROFJACLEVEL_UP(1:N_GEOMETRIES,0:NLAYERS,N,Q)
                 PROFJACLEVEL_DN_BAS(1:N_GEOMETRIES,0:NLAYERS,N,Q,T) = PROFJACLEVEL_DN(1:N_GEOMETRIES,0:NLAYERS,N,Q)
              endif
              PROFJACFLUXES_TOA_BAS(1:NBEAMS,:,N,Q,T) = PROFJACFLUXES_TOA(1:NBEAMS,:,N,Q)
              PROFJACFLUXES_BOA_BAS(1:NBEAMS,:,N,Q,T) = PROFJACFLUXES_BOA(1:NBEAMS,:,N,Q)
           ENDDO
        ENDDO

        Q = 1
        SURFACEWF_TOA_BAS(1:N_GEOMETRIES,Q,T) = SURFACEWF_TOA(1:N_GEOMETRIES,Q)
        SURFACEWF_BOA_BAS(1:N_GEOMETRIES,Q,T) = SURFACEWF_BOA(1:N_GEOMETRIES,Q)
        if ( do_2s_LEVELOUT ) THEN
           SURFJACLEVEL_UP_BAS(1:N_GEOMETRIES,0:NLAYERS,Q,T) = SURFJACLEVEL_UP(1:N_GEOMETRIES,0:NLAYERS,Q)
           SURFJACLEVEL_DN_BAS(1:N_GEOMETRIES,0:NLAYERS,Q,T) = SURFJACLEVEL_DN(1:N_GEOMETRIES,0:NLAYERS,Q)
        endif
        SURFJACFLUXES_TOA_BAS(1:NBEAMS,:,Q,T) = SURFJACFLUXES_TOA(1:NBEAMS,:,Q)
        SURFJACFLUXES_BOA_BAS(1:NBEAMS,:,Q,T) = SURFJACFLUXES_BOA(1:NBEAMS,:,Q)
...
!  ===========================================================
      ...
      write(*,*)
      if (do_brdf_surface) then
        write(*,*) 'Doing FD calculation 1: perturb Cox-Munk slope parameter'
      else
        write(*,*) 'Doing FD calculation 1: perturb Lambertian albedo'
      endif
...
!  Save results
        ...
        Q = 1
        INTENSITY_TOA_SURF_PT(1:N_GEOMETRIES,Q,T) = INTENSITY_TOA(1:N_GEOMETRIES)
        INTENSITY_BOA_SURF_PT(1:N_GEOMETRIES,Q,T) = INTENSITY_BOA(1:N_GEOMETRIES)

        if ( do_2s_LEVELOUT ) THEN
           RADLEVEL_UP_SURF_PT(1:N_GEOMETRIES,0:NLAYERS,Q,T) = RADLEVEL_UP(1:N_GEOMETRIES,0:NLAYERS)
           RADLEVEL_DN_SURF_PT(1:N_GEOMETRIES,0:NLAYERS,Q,T) = RADLEVEL_DN(1:N_GEOMETRIES,0:NLAYERS)
        endif

        FLUXES_TOA_SURF_PT(1:NBEAMS,:,Q,T) = FLUXES_TOA(1:NBEAMS,:)
        FLUXES_BOA_SURF_PT(1:NBEAMS,:,Q,T) = FLUXES_BOA(1:NBEAMS,:)
...
!  ==============================================
      ...
      write(*,*)
      write(*,*)'Doing FD calculation 2: perturb mol abs tau'
...
!  Save results
          ...
          Q = 1
          INTENSITY_TOA_PROF_PT(1:N_GEOMETRIES,I,Q,T) = INTENSITY_TOA(1:N_GEOMETRIES)
          INTENSITY_BOA_PROF_PT(1:N_GEOMETRIES,I,Q,T) = INTENSITY_BOA(1:N_GEOMETRIES)

          if ( do_2s_LEVELOUT ) THEN
             RADLEVEL_UP_PROF_PT(1:N_GEOMETRIES,0:NLAYERS,I,Q,T) = RADLEVEL_UP(1:N_GEOMETRIES,0:NLAYERS)
             RADLEVEL_DN_PROF_PT(1:N_GEOMETRIES,0:NLAYERS,I,Q,T) = RADLEVEL_DN(1:N_GEOMETRIES,0:NLAYERS)
          endif

          FLUXES_TOA_PROF_PT(1:NBEAMS,:,I,Q,T) = FLUXES_TOA(1:NBEAMS,:)
          FLUXES_BOA_PROF_PT(1:NBEAMS,:,I,Q,T) = FLUXES_BOA(1:NBEAMS,:)
...
!  ===============================================
      ...
      write(*,*)
      write(*,*)'Doing FD calculation 3: perturb aero ext tau'
...
! mick fix 1/16/2015 - set N_GEOMETRIES by hand since TWOSTREAM_LPS_MASTER not called in this case
            ...
            INTENSITY_TOA_PROF_PT(1:N_GEOMETRIES,I,Q,T) = INTENSITY_TOA_BAS(1:N_GEOMETRIES,T)
            INTENSITY_BOA_PROF_PT(1:N_GEOMETRIES,I,Q,T) = INTENSITY_BOA_BAS(1:N_GEOMETRIES,T)

            if ( do_2s_LEVELOUT ) THEN
               RADLEVEL_UP_PROF_PT(1:N_GEOMETRIES,0:NLAYERS,I,Q,T) = RADLEVEL_UP_BAS(1:N_GEOMETRIES,0:NLAYERS,T)
               RADLEVEL_DN_PROF_PT(1:N_GEOMETRIES,0:NLAYERS,I,Q,T) = RADLEVEL_DN_BAS(1:N_GEOMETRIES,0:NLAYERS,T)
            endif

            FLUXES_TOA_PROF_PT(1:NBEAMS,:,I,Q,T) = FLUXES_TOA_BAS(1:NBEAMS,:,T)
            FLUXES_BOA_PROF_PT(1:NBEAMS,:,I,Q,T) = FLUXES_BOA_BAS(1:NBEAMS,:,T)
          else
...
!  Initialize saved output for Fluxes and associated Jacobians
      ...
      write(*,*)'*******************'
      write(*,*)
      write(*,*)'Column WF baseline: Intensities, 2 Column WFs + 1 Surface WF'
...
!  Save Baseline results from this call
        ...
        DO Q = 1, N_COLUMN_WFS
           COLUMNWF_TOA_BAS(1:N_GEOMETRIES,Q,T) = COLUMNWF_TOA(1:N_GEOMETRIES,Q)
           COLUMNWF_BOA_BAS(1:N_GEOMETRIES,Q,T) = COLUMNWF_BOA(1:N_GEOMETRIES,Q)
           if ( do_2s_LEVELOUT ) THEN
              COLJACLEVEL_UP_BAS(1:N_GEOMETRIES,0:NLAYERS,Q,T) = COLJACLEVEL_UP(1:N_GEOMETRIES,0:NLAYERS,Q)
              COLJACLEVEL_DN_BAS(1:N_GEOMETRIES,0:NLAYERS,Q,T) = COLJACLEVEL_DN(1:N_GEOMETRIES,0:NLAYERS,Q)
           endif
           COLJACFLUXES_TOA_BAS(1:NBEAMS,:,Q,T) = COLJACFLUXES_TOA(1:NBEAMS,:,Q)
           COLJACFLUXES_BOA_BAS(1:NBEAMS,:,Q,T) = COLJACFLUXES_BOA(1:NBEAMS,:,Q)
        ENDDO
...
!  Save results
        ...
        Q = 1
        INTENSITY_TOA_COL_PT(1:N_GEOMETRIES,Q,T) = INTENSITY_TOA(1:N_GEOMETRIES)
        INTENSITY_BOA_COL_PT(1:N_GEOMETRIES,Q,T) = INTENSITY_BOA(1:N_GEOMETRIES)

        if ( do_2s_LEVELOUT ) THEN
           RADLEVEL_UP_COL_PT(1:N_GEOMETRIES,0:NLAYERS,Q,T) = RADLEVEL_UP(1:N_GEOMETRIES,0:NLAYERS)
           RADLEVEL_DN_COL_PT(1:N_GEOMETRIES,0:NLAYERS,Q,T) = RADLEVEL_DN(1:N_GEOMETRIES,0:NLAYERS)
        endif

        FLUXES_TOA_COL_PT(1:NBEAMS,:,Q,T) = FLUXES_TOA(1:NBEAMS,:)
        FLUXES_BOA_COL_PT(1:NBEAMS,:,Q,T) = FLUXES_BOA(1:NBEAMS,:)
...
!  Close output files
      ...
      close(36)
      close(37)
      if ( do_2S_LEVELOUT ) close(336)
      if ( do_2S_LEVELOUT ) close(337)
...
!    Update first line, Add extra line with current timing checks
      ...
      k0 = n_user_obsgeoms
      if (      do_user_obsgeoms ) open(667,file='Timing_ObsGeom_LPS',status='unknown')
      if ( .not.do_user_obsgeoms ) open(667,file='Timing_Lattice_LPS',status='unknown')
      read(667,'(6i2)')(nruns(n),n=1,6)
      nruns(k0)=nruns(k0)+1
      do n = 1, 6
         L = 0
         if ( n.eq.k0) then
...
!  (1) TOA file
      if (v == 1) then
        write(36,*)
        write(36,'(/T16,a/T16,a/)') &
          'INTENSITY, 1 SURFACE JACOBIAN, 2 PROFILE JACOBIANS', &
          '=================================================='
      end if

      write(36,'(/T16,a,I2,/T16,a/)')' ALL OUTPUT , threads 1-6, GEOMETRY # ',V, &
                                     '========================================='
      write(36,'(T16,6(6x,a16,6x)/)') &
        '   Solar No DM  ','  Solar Yes DM  ', &
        '  Thermal No DM ',' Thermal Yes DM ', &
        ' Solar+Thermal N',' Solar+Thermal Y'

      write(36,366) 'Intensity' , (INTENSITY_TOA_BAS(V,t),t=1,nthreads)

      write(36,*)
      q=1 !wrt Cox-Munk slope squared parameter
      write(36,367) 'Surface WF', &
                    (BRDF_PAR*surfacewf_TOA_BAS(V,q,t),&
                     (INTENSITY_TOA_SURF_PT(V,q,t)-INTENSITY_TOA_BAS(V,t))/eps,&
                     t=1,nthreads)

      write(36,*)
      do q=1,2 !wrt (1) mol abs tau & (2) aero ext tau
        do n = 1, nlayers
          write(36,368) 'Profile WF # ' ,n, &
                        (profilewf_TOA_BAS(V,n,q,t),&
                         (INTENSITY_TOA_PROF_PT(V,n,q,t)-INTENSITY_TOA_BAS(V,t))/eps,&
                         t=1,nthreads)
        enddo
        write(36,*)
      enddo
...
!  (2) BOA file
      if (v == 1) then
        write(37,*)
        write(37,'(/T16,a/T16,a/)') &
          'INTENSITY, 1 SURFACE JACOBIAN, 2 PROFILE JACOBIANS', &
          '=================================================='
      end if

      write(37,'(/T16,a,I2,/T16,a/)')' ALL OUTPUT , threads 1-6, GEOMETRY # ',V, &
                                     '========================================='
      write(37,'(T16,6(6x,a16,6x)/)') &
        '   Solar No DM  ','  Solar Yes DM  ', &
        '  Thermal No DM ',' Thermal Yes DM ', &
        ' Solar+Thermal N',' Solar+Thermal Y'

      write(37,366) 'Intensity' , (INTENSITY_BOA_BAS(V,t),t=1,nthreads)

      write(37,*)
      q=1 !wrt Cox-Munk slope squared parameter
      write(37,367) 'Surface WF', &
                    (BRDF_PAR*surfacewf_BOA_BAS(V,q,t),&
                     (INTENSITY_BOA_SURF_PT(V,q,t)-INTENSITY_BOA_BAS(V,t))/eps,&
                     t=1,nthreads)

      write(37,*)
      do q=1,2 !wrt (1) mol abs tau & (2) aero ext tau
        do n = 1, nlayers
          write(37,368) 'Profile WF # ' ,n, &
                        (profilewf_BOA_BAS(V,n,q,t),&
                         (INTENSITY_BOA_PROF_PT(V,n,q,t)-INTENSITY_BOA_BAS(V,t))/eps,&
                         t=1,nthreads)
        enddo
        write(37,*)
      enddo
...
!  (1) TOA file
      ...
      write(36,'(/T16,a,I2,/T16,a/)')' ALL OUTPUT , threads 1-2, BEAM # ',V, &
                                     '====================================='
      write(36,'(T16,2(6x,a16,6x)/)') &
        '   Solar No DM  ','  Solar Yes DM  '

      write(36,366) 'Flux' , (FLUXES_TOA_BAS(V,f,t),t=1,2)

      write(36,*)
      q=1 !wrt Cox-Munk slope squared parameter
      write(36,367) 'Surface WF', &
                    ( BRDF_PAR*SURFJACFLUXES_TOA_BAS(V,f,q,t),&
                      (FLUXES_TOA_SURF_PT(V,f,q,t)-FLUXES_TOA_BAS(V,f,t))/eps,t=1,2 )

      write(36,*)
      do q=1,2 !wrt (1) mol abs tau & (2) aero ext tau
        do n = 1, nlayers
          write(36,368) 'Profile WF # ' ,n, &
                        ( PROFJACFLUXES_TOA_BAS(V,f,n,q,t),&
                          (FLUXES_TOA_PROF_PT(V,f,n,q,t)-FLUXES_TOA_BAS(V,f,t))/eps,t=1,2 )
        enddo
        write(36,*)
      enddo
...
!  (2) BOA file
      ...
      write(37,'(/T16,a,I2,/T16,a/)')' ALL OUTPUT , threads 1-2, BEAM # ',V, &
                                     '====================================='
      write(37,'(T16,2(6x,a16,6x)/)') &
        '   Solar No DM  ','  Solar Yes DM  '

      write(37,366) 'Flux' , (FLUXES_BOA_BAS(V,f,t),t=1,2)

      write(37,*)
      q=1 !wrt Cox-Munk slope squared parameter
      write(37,367) 'Surface WF', &
                    ( BRDF_PAR*SURFJACFLUXES_BOA_BAS(V,f,q,t),&
                      (FLUXES_BOA_SURF_PT(V,f,q,t)-FLUXES_BOA_BAS(V,f,t))/eps,t=1,2 )

      write(37,*)
      do q=1,2 !wrt (1) mol abs tau & (2) aero ext tau
        do n = 1, nlayers
          write(37,368) 'Profile WF # ' ,n, &
                        ( PROFJACFLUXES_BOA_BAS(V,f,n,q,t),&
                          (FLUXES_BOA_PROF_PT(V,f,n,q,t)-FLUXES_BOA_BAS(V,f,t))/eps,t=1,2 )
        enddo
        write(37,*)
      enddo
...
!  (1) TOA file
      if (v == 1) then
        write(336,*)
        write(336,'(/T16,a/T16,a/)') &
          'INTENSITY, 1 SURFACE JACOBIAN, 2 PROFILE JACOBIANS', &
          '=================================================='
      end if

      write(336,'(/T33,a,I2,/T33,a/)')' ALL OUTPUT , threads 1-6, GEOMETRY # ',V, &
                                     '========================================='
      write(336,'(T33,6(6x,a16,6x)/)') &
        '   Solar No DM  ','  Solar Yes DM  ', &
        '  Thermal No DM ',' Thermal Yes DM ', &
        ' Solar+Thermal N',' Solar+Thermal Y'

      do n = 0, nlayers
         write(336,3366)'Intensity @ level # ',DBLE(N), (RADLEVEL_UP_BAS(V,N,T),T=1,NTHREADS)
      enddo

      write(336,*)
      q=1 !wrt Cox-Munk slope squared parameter
      do n = 0, nlayers
         write(336,3367) 'Surface WF @ level #', DBLE(N), &
                    (BRDF_PAR*SURFJACLEVEL_UP_BAS(V,n,q,t),&
                     (RADLEVEL_UP_SURF_PT(V,n,q,t)-RADLEVEL_UP_BAS(V,n,t))/eps,&
                     t=1,nthreads)
      enddo
      ...
      do n = 0, nlayers
         do q=1,2 !wrt (1) mol abs tau & (2) aero ext tau
            do k = 1, nlayers
               write(336,3368) 'Profile WF # ',q,' @ level #',k,DBLE(N), &
                        (PROFJACLEVEL_UP_BAS(V,n,k,q,t),&
                         (RADLEVEL_UP_PROF_PT(V,n,k,q,t)-RADLEVEL_UP_BAS(V,n,t))/eps,&
                         t=1,nthreads)
            enddo
            write(336,*)
         enddo
...
!  (2) BOA file
      if (v == 1) then
        write(337,*)
        write(337,'(/T16,a/T16,a/)') &
          'INTENSITY, 1 SURFACE JACOBIAN, 2 PROFILE JACOBIANS', &
          '=================================================='
      end if

      write(337,'(/T33,a,I2,/T33,a/)')' ALL OUTPUT , threads 1-6, GEOMETRY # ',V, &
                                     '========================================='
      write(337,'(T33,6(6x,a16,6x)/)') &
        '   Solar No DM  ','  Solar Yes DM  ', &
        '  Thermal No DM ',' Thermal Yes DM ', &
        ' Solar+Thermal N',' Solar+Thermal Y'

      do n = 0, nlayers
         write(337,3366)'Intensity @ level #',DBLE(N), (RADLEVEL_DN_BAS(V,N,T),T=1,NTHREADS)
      enddo

      write(337,*)
      q=1 !wrt Cox-Munk slope squared parameter
      do n = 0, nlayers
         write(337,3367) 'Surface WF @ level #', DBLE(N), &
                    (BRDF_PAR*SURFJACLEVEL_DN_BAS(V,n,q,t),&
                     (RADLEVEL_DN_SURF_PT(V,n,q,t)-RADLEVEL_DN_BAS(V,n,t))/eps,&
                     t=1,nthreads)
      enddo
      ...
      do n = 0, nlayers
         do q=1,2 !wrt (1) mol abs tau & (2) aero ext tau
            do k = 1, nlayers
               write(337,3368) 'Profile WF # ',q,' @ level #',k,DBLE(N), &
                        (PROFJACLEVEL_DN_BAS(V,n,k,q,t),&
                         (RADLEVEL_DN_PROF_PT(V,n,k,q,t)-RADLEVEL_DN_BAS(V,n,t))/eps,&
                         t=1,nthreads)
            enddo
            write(337,*)
         enddo
...
!  (1) TOA file
      ...
      write(36,*)
      do q=1,2 !wrt (1) mol abs tau & (2) aero ext tau
        write(36,368) 'Column WF # ' ,q, &
                      (columnwf_TOA_BAS(V,q,t),&
                       (INTENSITY_TOA_COL_PT(V,q,t)-INTENSITY_TOA_BAS(V,t))/eps,&
                       t=1,nthreads)
      enddo
...
!  (2) BOA file
      ...
      write(37,*)
      do q=1,2 !wrt (1) mol abs tau & (2) aero ext tau
        write(37,368) 'Column WF # ' ,q, &
                      (columnwf_BOA_BAS(V,q,t),&
                       (INTENSITY_BOA_COL_PT(V,q,t)-INTENSITY_BOA_BAS(V,t))/eps,&
                       t=1,nthreads)
      enddo
...
!  (1) TOA file
      ...
      do q=1,2 !wrt (1) mol abs tau & (2) aero ext tau
          write(36,368) 'Column WF # ' ,q,&
                        ( COLJACFLUXES_TOA_BAS(V,f,q,t),&
                          (FLUXES_TOA_COL_PT(V,f,q,t)-FLUXES_TOA_BAS(V,f,t))/eps,t=1,2 )
      enddo
...
!  (2) BOA file
      ...
      do q=1,2 !wrt (1) mol abs tau & (2) aero ext tau
        write(37,368) 'Column WF # ',q, &
                      ( COLJACFLUXES_BOA_BAS(V,f,q,t),&
                        (FLUXES_BOA_COL_PT(V,f,q,t)-FLUXES_BOA_BAS(V,f,t))/eps,t=1,2 )
      enddo
...
!  (1) TOA file
      ...
      do n = 0, nlayers
         write(336,3366)'Level Intensity @ level #',DBLE(N), (RADLEVEL_UP_BAS(V,N,T),T=1,NTHREADS)
      enddo
      ...
      do n = 0, nlayers
         do q=1,2 !wrt (1) mol abs tau & (2) aero ext tau
            write(336,3368) 'Column WF ',q,' @ level #', DBLE(N), &
                        (COLJACLEVEL_UP_BAS(V,n,q,t),&
                         (RADLEVEL_UP_COL_PT(V,n,q,t)-RADLEVEL_UP_BAS(V,n,t))/eps,&
                         t=1,nthreads)
         enddo
      enddo
...
!  (2) BOA file
      ...
      do n = 0, nlayers
         do q=1,2 !wrt (1) mol abs tau & (2) aero ext tau
            write(337,3368) 'Column WF ',q,' @ level #' ,DBLE(N), &
                        (COLJACLEVEL_DN_BAS(V,n,q,t),&
                         (RADLEVEL_DN_COL_PT(V,n,q,t)-RADLEVEL_DN_BAS(V,n,t))/eps,&
                         t=1,nthreads)
         enddo
      enddo
