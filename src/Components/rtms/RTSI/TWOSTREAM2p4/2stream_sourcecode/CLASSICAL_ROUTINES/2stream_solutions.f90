...
SUBROUTINE TWOSTREAM_HOM_SOLUTION &
          ( MAXLAYERS,                               & ! Dimensions
            N, FOURIER, STREAM_VALUE, PXSQ,          & ! Inputs
...
!  coefficient functions for user-defined angles
      ...
      REAL(kind=dp), INTENT(OUT) :: ZETA_P(MAX_USER_STREAMS,MAXLAYERS)
      REAL(kind=dp), INTENT(OUT) :: ZETA_M(MAX_USER_STREAMS,MAXLAYERS)
...
!      REAL(kind=dp), INTENT(INOUT) :: U_WPOS1(MAX_USER_STREAMS,MAXLAYERS)
!      REAL(kind=dp), INTENT(INOUT) :: U_WNEG1(MAX_USER_STREAMS,MAXLAYERS)

!  Diffuse-term Particular beam solutions at user-defined angles

      REAL(kind=dp), INTENT(INOUT) :: U_WPOS2(MAX_USER_STREAMS,MAXLAYERS)
      REAL(kind=dp), INTENT(INOUT) :: U_WNEG2(MAX_USER_STREAMS,MAXLAYERS)
...
!  Distinguish between upwelling and downwelling
      ...
      IF ( DO_UPWELLING ) THEN
        IF ( DO_USER_OBSGEOMS ) THEN
          if (fourier.eq.0 ) then
            pos2 = help2(0) + help2(1) * user_streams(IBEAM)
          else
            pos2 = help2(1) * ULP(IBEAM)
          endif
          U_WPOS2(IBEAM,N) = POS2
        ELSE
          DO UM = 1, N_USER_STREAMS
            if (fourier.eq.0 ) then
