...
module twostream_master_WITHLAPACK_m

Use twostream_LAPACK_m
Use twostream_miscsetups_m
Use twostream_solutions_m
Use twostream_bvproblem_WITHLAPACK_m
...
SUBROUTINE TWOSTREAM_MASTER_WITHLAPACK &
        ( MAXLAYERS, MAXTOTAL, MAXTHREADS, MAXMESSAGES,                   & ! Dimensions
          ...
          DO_SURFACE_LEAVING, DO_SL_ISOTROPIC,                            & ! Inputs     !@@ 2p3
          ...
          DO_PENTADIAG_INVERSE, BVPINDEX, BVPSCALEFACTOR,                 & ! Inputs     !@@ 2p3 6/25/14
...
!  Notes 05 November 2013. Flux output options. Two New Flgas
!   DO_MVOUT_ONLY
!   DO_ADDITIONAL_MVOUT
...
!      !@@ MAX_USER_OBSGEOMS >/= MAXBEAMS
      ...
      INTEGER, INTENT(IN)        :: MAXLAYERS, MAXTOTAL !, MAXTOTAL1, MAXTOTAL2
...
!  !@@ Call statement expanded to include BVProblem control         !@@ 2p3  06/25/14
        ...
        ( MAXLAYERS, MAXTOTAL, MAXBEAMS, MAX_USER_STREAMS, MAXTHREADS,   & ! Dimensions
          ...
          DO_MVOUT_ONLY, DO_ADDITIONAL_MVOUT, DO_POSTPROCESSING,         & ! Input !@@ New line, 2p3
          DO_SURFACE_LEAVING, DO_SL_ISOTROPIC,                           & ! Input !@@ New line, 2p3
          ...
          SLTERM_ISOTROPIC, SLTERM_F_0,                                  & ! Input real SLEAVE @@ 2p3 1/23/14
          ...
          TCOM1, DELTAU_POWER, BMAT_ROWMASK,                             & ! In/Out
...
!  Finish
      ...
      RETURN
END SUBROUTINE TWOSTREAM_MASTER_WITHLAPACK
...
!  Boundary Value Problem
!  Bandmatrix and pivot, compression, etc (LAPACK option)

      REAL(kind=dp) :: BANDMAT2(7,MAXTOTAL)
      REAL(kind=dp) :: SMAT2   (2,2)
      INTEGER       :: IPIVOT  (MAXTOTAL)

!  Pentadiagonal Matrix inverse for solving BCs (Module 2)
!      REAL(kind=dp) :: PHIN(MAXTOTAL1,MAXTOTAL1)
...
!     LAPACK Option re-introduced, 08 April 2014
!     Pentadiagonal inverse option introduced, 24 June 2014

      IF ( BVPINDEX .eq. 0 ) THEN
        CALL TWOSTREAM_BVP_MATSETUP_LAPACK &
         ( MAXLAYERS, MAXTOTAL, DO_INCLUDE_SURFACE,                 & ! input
           BVPSCALEFACTOR, FOURIER, NLAYERS, NTOTAL,                & ! input
      ...
      ELSE IF ( BVPINDEX .eq. 1 ) THEN
        CALL TWOSTREAM_BVP_MATSETUP_PENTADIAG &
         ( MAXLAYERS, MAXTOTAL, BVPSCALEFACTOR, DO_PENTADIAG_INVERSE, & ! Dimensions
           ...
           STATUS_SUB, MESSAGE )                                        ! Output
!      ELSE IF ( BVPINDEX .eq. 2 ) THEN
!         CALL TWOSTREAM_BVP_MATSETUP_PENTADIAG_2 &
!         ( MAXLAYERS, MAXTOTAL, MAXTOTAL1, MAXTOTAL2,               & ! Dimensions
!           DO_INCLUDE_SURFACE, FOURIER, NLAYERS, NTOTAL,            & ! input
!           DO_BRDF_SURFACE, SURFACE_FACTOR, ALBEDO, BRDF_F,         & ! input
!           XPOS, XNEG, EIGENTRANS, STREAM_VALUE,                    & ! input
!           H_HOMP, H_HOMM, PHIN, SMAT,                              & ! output
!           STATUS_SUB, MESSAGE )                                      ! output
      ENDIF

!  Exception handling for Matrix Setup (Pentadiagonal or LAPACK)

      IF ( STATUS_SUB .NE. 0 ) THEN
        IF ( BVPINDEX .eq. 0 ) THEN
           TRACE  = 'Call BVP_MATSETUP_LAPACK in 2S_FOURIER_MASTER'
        ELSE IF ( BVPINDEX .eq. 1 ) THEN
...
!     Pentadiagonal inverse option introduced, 25 June 2014
      ...
      IF ( BVPINDEX .eq. 0  ) THEN
         CALL TWOSTREAM_BVP_SOLUTION_LAPACK &
            ( MAXLAYERS, MAXBEAMS, MAXTOTAL, BVPSCALEFACTOR,   & ! Dimensions + BVP control
      ...
      ELSE IF ( BVPINDEX .eq. 1 ) THEN
         CALL TWOSTREAM_BVP_SOLUTION_PENTADIAG &
          ( MAXLAYERS, MAXBEAMS, MAXTOTAL,                   & ! Dimensions
            ...
            H_PARTIC, LCON, MCON, LCON_XVEC, MCON_XVEC )       ! Output
!      ELSE IF ( BVPINDEX .eq. 2 ) THEN
!        CALL TWOSTREAM_BVP_SOLUTION_PENTADIAG_2 &
!         ( MAXLAYERS, MAXBEAMS, MAXTOTAL, MAXTOTAL1,        & ! Dimensions
!           DO_INCLUDE_SURFACE, DO_INCLUDE_DIRECTBEAM,       & ! inputs
!           DO_INCLUDE_SURFEMISS, DO_BRDF_SURFACE,           & ! inputs
!           FOURIER, IBEAM, NLAYERS, NTOTAL,                 & ! inputs
!           SURFACE_FACTOR, ALBEDO, BRDF_F, EMISS, SURFBB,   & ! inputs
!           DIRECT_BEAM, XPOS, XNEG, WUPPER, WLOWER,         & ! inputs
!           STREAM_VALUE, PHIN, SMAT,                        & ! inputs
!           H_PARTIC, LCON, MCON, LCON_XVEC, MCON_XVEC  )      ! Output
      ENDIF

!  Exception handling for BVP Solution (LAPACK)

      IF ( BVPINDEX .eq. 0 .and. STATUS_SUB .NE. 0 ) THEN
         TRACE  = 'Thermal, Call to BVP_SOLUTION_LAPACK in 2S_FOURIER_MASTER'
         STATUS = 1 ; RETURN
      ENDIF
...
!  Exception handling for BVP Solution (LAPACK)
        ...
        IF ( BVPINDEX .eq. 0 .and. STATUS_SUB .NE. 0 ) THEN
           TRACE  = 'Solar, Call to BVP_SOLUTION_LAPACK in 2S_FOURIER_MASTER'
           STATUS = 1 ; RETURN
        ENDIF
...
end module twostream_master_WITHLAPACK_m
