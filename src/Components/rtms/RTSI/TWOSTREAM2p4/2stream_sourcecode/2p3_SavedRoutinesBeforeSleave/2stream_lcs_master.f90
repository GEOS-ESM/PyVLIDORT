...
SUBROUTINE TWOSTREAM_LCS_MASTER &
        ( MAXLAYERS, MAXTOTAL, MAXTHREADS, MAXMESSAGES,                    & ! Dimensions
          ...
          DO_SIM_ONLY, DO_SURFACE_WFS, N_COLUMN_WFS, N_SURFACE_WFS,        & ! Inputs
...
!  Main output
      ...
      COLUMNWF_TOA(:,:,T)  = 0.0d0
      COLUMNWF_BOA(:,:,T)  = 0.0d0
...
! @@ Rob Spurr, 17 July 2013, Version 2.2 --> Optional Output at ALL LEVELS
      ...
      COLJACLEVEL_UP (:,:,:,T) = 0.0d0
      COLJACLEVEL_DN (:,:,:,T) = 0.0d0
...
! @@ Rob Spurr, 05 November 2013, Version 2.3 --> BOA_TOA Flux outputs
      ...
      COLJACFLUXES_TOA (:,:,:,T) = 0.0d0
      COLJACFLUXES_BOA (:,:,:,T) = 0.0d0
...
!  !@@ Call statement expanded to include Flux outputs and control !@@ 2p3  11/5/13
        ...
        ( MAXLAYERS, MAXTOTAL, MAXBEAMS, MAX_USER_STREAMS, MAXTHREADS,         & ! Dimensions
          ...
          DO_MVOUT_ONLY, DO_ADDITIONAL_MVOUT, DO_POSTPROCESSING,                & ! Input !@@ New line, 2p3
          NLAYERS, NTOTAL, NBEAMS, N_USER_STREAMS, N_COLUMN_WFS, N_SURFACE_WFS, & ! Input integer control
          ...
          LC_INITIAL_TRANS, LC_AVERAGE_SECANT, LC_T_DELT_MUBAR,                 & ! In/Out
          L_T_DELT_USERM, LC_EMULT_UP, LC_EMULT_DN,                             & ! In/Out
...
!  Exception handling
        ...
        IF ( STATUS_SUB .NE. 0 ) THEN
          STATUS_EXECUTION = 1
          WRITE(CF,'(I2)')FOURIER
          E_TRACE_2 = 'Error from 2S_LCS_FOURIER_MASTER, Fourier # ' &
                        //CF//', Thread # '//wthread
          RETURN
        ENDIF
...
!  Linearized Beam solution
      ...
      REAL(kind=dp) :: LC_WVEC(2,MAXLAYERS,MAX_ATMOSWFS)
...
!  Diffuse-term Particular beam solutions at user-defined angles
      ...
      REAL(kind=dp) ::  LC_U_WPOS2(MAX_USER_STREAMS,MAXLAYERS,MAX_ATMOSWFS)
      REAL(kind=dp) ::  LC_U_WNEG2(MAX_USER_STREAMS,MAXLAYERS,MAX_ATMOSWFS)

!  Single-scatter Particular beam solutions at user-defined angles
!    NOT REQUIRED, MS_MODE only
!      REAL(kind=dp) ::  LC_U_WPOS1(MAX_USER_STREAMS,MAXLAYERS,MAX_ATMOSWFS)
!      REAL(kind=dp) ::  LC_U_WNEG1(MAX_USER_STREAMS,MAXLAYERS,MAX_ATMOSWFS)
...
!     ! @@ Rob Spurr, 17 July 2013, Version 2.2 --> Optional output at ALL LEVELS
      ...
      COLJACLEVEL_F_UP = 0.0d0
      COLJACLEVEL_F_DN = 0.0d0
...
!  Local linearization flag
      ...
      IF ( do_COLUMN_WFS ) THEN
         ...
         LAYER_VARY_NUMBER(1:NLAYERS) = N_COLUMN_WFS
      ENDIF
        ...
        IF ( DO_INCLUDE_THERMEMISS ) THEN
          IF ( DO_COLUMN_WFS  ) THEN
            ...
            ( MAXLAYERS, MAX_ATMOSWFS,                                 & ! Dimensions
              ...
              DO_COLUMN_WFS, LAYER_VARY_FLAG, LAYER_VARY_NUMBER,       & ! Inputs
...
!      !@@ Add alternative for Observational geometry
          ...
          IF ( DO_COLUMN_WFS ) THEN
            IF ( DO_USER_OBSGEOMS ) THEN
              CALL TWOSTREAM_LC_EMULTMASTER_OBSGEO &
              ( MAXLAYERS, MAXBEAMS, MAX_USER_STREAMS, MAX_ATMOSWFS,  & ! Dimensions
      ...
      IF ( DO_INCLUDE_THERMEMISS ) THEN
        ...
        IF ( DO_COLUMN_WFS ) THEN
          ...
          ( MAXLAYERS, MAX_USER_STREAMS, MAX_ATMOSWFS,           & ! Dimensions
            ...
            DO_COLUMN_WFS, LAYER_VARY_FLAG, LAYER_VARY_NUMBER,   & ! Input
...
!  Solve the linearized boundary value problem
        ...
        CALL TWOSTREAM_BVP_LC_SOLUTION_MASTER &
            ( MAXLAYERS, MAXTOTAL, MAXBEAMS, MAX_ATMOSWFS,                   & ! Dimensions
              DBEAM, DO_INCLUDE_THERMEMISS, DO_SOLAR_SOURCES,                & ! inputs
              DO_INCLUDE_SURFACE, DO_BRDF_SURFACE,                           & ! inputs
              FOURIER, IBEAM, NLAYERS, NTOTAL, N_COLUMN_WFS,                 & ! inputs
              ...
              L_DELTAU_VERT, LC_INITIAL_TRANS, LC_T_DELT_MUBAR,              & ! inputs
              L_EIGENTRANS, L_XPOS, L_XNEG, LC_WVEC, L_T_WUPPER, L_T_WLOWER, & ! inputs
...
!         !@@ 2p3. 11/5/13. Post-processing control
        ...
        IF ( DO_UPWELLING .and. DO_POSTPROCESSING ) THEN
          ...
          ( MAXLAYERS, MAXBEAMS, MAX_USER_STREAMS, MAX_ATMOSWFS,            & ! Dimensions
            ...
            FOURIER, IBEAM, NLAYERS, N_USER_STREAMS, N_COLUMN_WFS, PI4,     & ! input
...
!         !@@ 2p3. 11/5/13. Post-processing control
        ...
        IF ( DO_DNWELLING .and. DO_POSTPROCESSING) THEN
          ...
          ( MAXLAYERS, MAXBEAMS, MAX_USER_STREAMS, MAX_ATMOSWFS,       & ! Dimensions
            DO_INCLUDE_THERMEMISS, DO_SOLAR_SOURCES,                   & ! Dimensions
            DO_USER_OBSGEOMS, DO_2S_LEVELOUT,                          & ! Inputs !@@ 2p1, 2p2
            FOURIER, IBEAM, NLAYERS, N_USER_STREAMS, N_COLUMN_WFS,     & ! input
...
!  Linearized solution
          ...
          IF ( DO_COLUMN_WFS ) THEN
            CALL TWOSTREAM_LC_BEAM_SOLUTION &
