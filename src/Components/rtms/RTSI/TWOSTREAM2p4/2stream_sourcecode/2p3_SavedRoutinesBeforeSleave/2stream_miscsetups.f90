...
!  Dimensions :
      ...
      INTEGER, INTENT(IN) :: MAXLAYERS, MAXMESSAGES, MAXTHREADS
...
!  Optical properties
      ...
      REAL(kind=dp), INTENT(IN) :: DELTAU_VERT (MAXLAYERS, MAXTHREADS )
      REAL(kind=dp), INTENT(IN) :: OMEGA_TOTAL (MAXLAYERS, MAXTHREADS )
      REAL(kind=dp), INTENT(IN) :: ASYMM_TOTAL (MAXLAYERS, MAXTHREADS )
...
!  Solar beam attenuations
      ...
      REAL(kind=dp), INTENT(OUT) :: SOLAR_BEAM_OPDEP ( MAXBEAMS )
...
!  !@@ Option for Observational Goemetry, 12/21/12.
      ...
      IF ( DO_USER_OBSGEOMS ) THEN
        DO IB = 1, NBEAMS
          DO N = 1, NLAYERS
            ITRANS_USERM(N,LUM,IB) = INITIAL_TRANS(N,IB)/USER_STREAMS(IB)
          ENDDO
        ENDDO
      ELSE
        DO IB = 1, NBEAMS
...
!  Solar beam attenuations and reflectance flags
      ...
      REAL(kind=dp), INTENT(IN)  :: SOLAR_BEAM_OPDEP        ( MAXBEAMS )
...
SUBROUTINE TWOSTREAM_EMULTMASTER_OBSGEO &
           ( MAXLAYERS, MAXBEAMS, MAX_USER_STREAMS,          & ! Dimensions
             DO_UPWELLING, DO_DNWELLING,                     & ! Input
...
!  ----------------------------------------------
      ...
      IF ( DO_UPWELLING ) THEN
        DO N = 1, NLAYERS
          DO IB = 1, NBEAMS
            IF ( N .GT. LAYER_PIS_CUTOFF(IB) ) THEN
              EMULT_UP(LUM,N,IB) = 0.0_dp
            ELSE
              WDEL = T_DELT_MUBAR(N,IB)
              WUDEL = WDEL * T_DELT_USERM(N,IB)
              SU = ( 1.0_dp - WUDEL ) / SIGMA_P(N,LUM,IB)
...
!  Finish
      ...
      RETURN
END SUBROUTINE TWOSTREAM_EMULTMASTER_OBSGEO
