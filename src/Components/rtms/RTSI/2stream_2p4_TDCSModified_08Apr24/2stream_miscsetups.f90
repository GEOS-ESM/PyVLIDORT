...
SUBROUTINE TWOSTREAM_CHECK_INPUTS_BASIC &
         ( MAXLAYERS, MAXMESSAGES, MAX_USER_OBSGEOMS,                   & ! Dimensions !@@
           ...
           DO_UPWELLING, DO_DNWELLING, DO_PLANE_PARALLEL, DO_DMSCALING, & ! inputs
           ...
           STREAM_VALUE, BEAM_SZAS, USER_ANGLES, USER_RELAZMS,          & ! inputs
...
!  5/8/24. Introduce flag DO_HEMISPHER_MEAN for thermal regime, FLUXES_ONLY
!   -- Check that MUBAR = 0.5 and there is no solar scattering, no DELTAM_Scaling

!  5/9/24. Introduce flag DO_EDDINGTON_FLUX for solar regime, FLUXES-ONLY
!   -- Check that MUBAR = 0.5 and there is no thermal sources, no DELTAM_Scaling
...
!  Flags
      ...
      LOGICAL, INTENT(IN) :: DO_SOLAR_SOURCES, DO_THERMAL_EMISSION, DO_DMSCALING
...
!  6/29/24. Introduce flag DO_TRIDIAG
!             -- Checking Eddington Flux and Hemispheric mean options.
...
!  5/8/24. Introduce stream value for checking hemispheric mean option
...
!  5/8/24. Introduce flag DO_HEMISPHER_MEAN for thermal regime
!   -- Thermal only, mubar = 0.5, no deltam, fluxes only. TRIDIAG only (6/29/24)

      IF ( DO_HEMISPHER_MEAN ) then
        if ( DO_SOLAR_SOURCES ) then
          NM = NM + 1
          MESSAGE(NM) = 'Bad input: No solar sources for thermal-only run with DO_HEMISPHER_MEAN'
          ACTION(NM)  = 'Abort: must set DO_SOLAR_SOURCES = .false.'
          STATUS = 1
        ENDIF
        IF ( ABS(STREAM_VALUE-0.5_dp).gt.1.0D-15 ) then
          NM = NM + 1
          MESSAGE(NM) = 'Bad input: Stream Value not 0.5 for thermal-only run with DO_HEMISPHER_MEAN'
          ACTION(NM)  = 'Abort: must set STREAM_VALUE = 0.5'
          STATUS = 1
        ENDIF
        IF ( .not. DO_MVOUT_ONLY ) then
          NM = NM + 1
          MESSAGE(NM) = 'Bad input: Fluxes Alone for thermal-only run with DO_HEMISPHER_MEAN'
          ACTION(NM)  = 'Abort: must set DO_MVOUT_ONLY = .true.'
          STATUS = 1
        ENDIF
        IF ( DO_DMSCALING ) then
          NM = NM + 1
          MESSAGE(NM) = 'Bad input: No Deltam-Scaling for thermal-only run with DO_HEMISPHER_MEAN'
          ACTION(NM)  = 'Abort: must set DO_DELTAM_SCALING = .false.'
          STATUS = 1
        ENDIF
        IF ( .not. DO_TRIDIAG ) then
          NM = NM + 1
          MESSAGE(NM) = 'Bad input: Must use TdiDiagonal BVP solver with thermal-only DO_HEMISPHER_MEAN'
          ACTION(NM)  = 'Abort: must set DO_TRIDIAG = .false.'
          STATUS = 1
        ENDIF
      ENDIF

!  5/9/24. Introduce flag DO_EDDINGTON_FLUX for solar flux regime
!   -- Solar only, mubar = 0.5, no deltam, fluxes only. TRIDIAG only (6/29/24)

      IF ( DO_EDDINGTON_FLUX ) then
        if ( DO_THERMAL_EMISSION ) then
          NM = NM + 1
          MESSAGE(NM) = 'Bad input: No thermal sources for solar fluxes-only run with DO_EDDINGTON_FLUX'
          ACTION(NM)  = 'Abort: must set DO_SOLAR_SOURCES = .false.'
          STATUS = 1
        ENDIF
        IF ( ABS(STREAM_VALUE-0.5_dp).gt.1.0D-15 ) then
          NM = NM + 1
          MESSAGE(NM) = 'Bad input: Stream Value not 0.5 for solar fluxes-only run with DO_EDDINGTON_FLUX'
          ACTION(NM)  = 'Abort: must set STREAM_VALUE = 0.5'
          STATUS = 1
        ENDIF
        IF ( .not. DO_MVOUT_ONLY ) then
          NM = NM + 1
          MESSAGE(NM) = 'Bad input: Fluxes Alone for solar fluxes run with DO_EDDINGTON_FLUX'
          ACTION(NM)  = 'Abort: must set DO_MVOUT_ONLY = .true.'
          STATUS = 1
        ENDIF
        IF ( DO_DMSCALING ) then
          NM = NM + 1
          MESSAGE(NM) = 'Bad input: No Deltam-Scaling for solar fluxes-only run with DO_EDDINGTON_FLUX'
          ACTION(NM)  = 'Abort: must set DO_DELTAM_SCALING = .false.'
          STATUS = 1
        ENDIF
        IF ( .not. DO_TRIDIAG ) then
          NM = NM + 1
          MESSAGE(NM) = 'Bad input: Must use TdiDiagonal BVP solver with solar fluxes-only DO_EDDINGTON_FLUX'
          ACTION(NM)  = 'Abort: must set DO_TRIDIAG = .false.'
          STATUS = 1
        ENDIF
      ENDIF
...
!  local variables
      ...
      INTEGER           :: L, NM
      CHARACTER(LEN=3)  :: C3
      REAL(kind=dp), parameter :: OMEGATOGGLE = 1.0e-9_dp
...
!   If reached, make sure Classical PI is operational

      IF ( .not. DO_CLASSICAL_PI ) then
!        DO L = 1, NLAYERS
!          IF ( ABS ( 1.0_dp - OMEGA_TOTAL(L) ) .lt. OMEGATOGGLE  )  STATUS = 1
!        ENDDO
        IF (STATUS.eq.1 ) then
          NM = NM + 1
          MESSAGE(NM) = 'Bad input for PI Solution Method: At least 1 SS-albedos too close to 1'
          ACTION (NM) = 'Turn on DO_CLASSICAL_PI flag for atmosphere with any conservative scattering'
        ENDIF
     ENDIF
...
!  6/26/24. Include flag for Eddington flux solution
...
!  ---------------
      ...
      REAL(kind=dp) :: PIE, PI4, X0_FLUX, ATTN, REFL_ATTN, HELP, SL
      INTEGER       :: IB
...
!        ATTN           = X0_FLUX * TRANS_SOLAR_BEAM(IB)
        ATTN           = X0_FLUX * TRANS_SOLAR_BEAM(IB) / 2.0_dp / STREAM_VALUE
        ATMOS_ATTN(IB) = ATTN
...
!  6/29/24. Special consideration for Eddington solution (multiply by PI)

        IF ( DO_EDDINGTON_FLUX ) THEN
          DIRECT_BEAM(IB) = PIE * DIRECT_BEAM(IB)
        ENDIF
