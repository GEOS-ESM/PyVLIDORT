...
SUBROUTINE TWOSTREAM_UPUSER_INTENSITY &
  ( MAXLAYERS, MAXBEAMS, MAX_USER_STREAMS,                       & ! Dimensions
    ...
    T_DELT_EIGEN, LCON, LCON_XVEC, MCON, MCON_XVEC, WLOWER, TGM, CSPPHOMUP, & ! inputs
...
!  Tridiagonal flag, added 3/28/18 for radiance calculations with Tridiagonal BVP
...
!   Flag for using Conservative scattering solution in a given layer
!  Rob Fix 5/24/17 for 2p4a, Develop handles for Conservative scattering
...
!  Rob Fix 5/24/17 for 2p4a, Only needed for classical PI method
...
!  Conservative scattering solutions. Introduced 3/28/18

      REAL(kind=dp), INTENT(IN)  :: TGM(MAXLAYERS)
      REAL(kind=dp), INTENT(IN)  :: CSPPHOMUP(2,MAX_USER_STREAMS,MAXLAYERS)
...
!  help variables
      ...
      REAL(kind=dp) :: SHOM, SPAR, PAR, HOM, KMULT, TM, SU, SD, SHOM1,SHOM2,SHOM3,SHOM4
      REAL(kind=dp) :: FAC1, FAC2, EPS, ITRANS, ITRANSWDEL, WDEL, YFAC, MULT, SFOR2
...
!  Rob Fix 5/24/17 for 2p4a, Develop handles for Conservative scattering
!  Rob Fix 3/28/18 for 2p4a, Explicitly calculate Conservative scattering using TGM

        N = NLAYERS
        PAR = WLOWER(1,N)
        if ( CONSSCAT(N) ) THEN
          HOM = LCON(N) * ( ONE - TGM(N) ) + MCON(N) * TGM(N)
        else
...
!  Rob Fix 5/24/17 for 2p4a, Develop handles for Conservative scattering
!  Rob Fix 3/28/18 for 2p4a, Explicitly calculate Conservative scattering

        IF ( CONSSCAT(N) ) THEN
          DO LUM = 1, N_PPSTREAMS
            UM = PPSTREAM_MASK(LUM,IB)
            SHOM = LCON(N) * CSPPHOMUP(1,UM,N) + MCON(N) * CSPPHOMUP(2,UM,N) 
            LAYERSOURCE(UM) = SHOM
          ENDDO
        ELSE
          IF ( DO_TRIDIAG ) THEN
...
!  This group wrong.......
!              SHOM1 = LCON_XVEC(1,N) * U_XPOS(UM,N) * HMULT_1(UM,N) + &
!                     MCON_XVEC(1,N) * U_XPOS(UM,N) * HMULT_2(UM,N) + &
!                     LCON_XVEC(2,N) * U_XNEG(UM,N) * HMULT_2(UM,N) + &
!                     MCON_XVEC(2,N) * U_XNEG(UM,N) * HMULT_1(UM,N)

              SHOM2 = LCON_XVEC(1,N) * U_XNEG(UM,N) * HMULT_1(UM,N) + &
                     MCON_XVEC(1,N) * U_XNEG(UM,N) * HMULT_2(UM,N) + &
                     LCON_XVEC(2,N) * U_XPOS(UM,N) * HMULT_2(UM,N) + &
                     MCON_XVEC(2,N) * U_XPOS(UM,N) * HMULT_1(UM,N)

!  This group wrong.......
!              SHOM3 = LCON_XVEC(1,N) * U_XPOS(UM,N) * HMULT_2(UM,N) + &
!                     MCON_XVEC(1,N) * U_XPOS(UM,N) * HMULT_1(UM,N) + &
!                     LCON_XVEC(2,N) * U_XNEG(UM,N) * HMULT_1(UM,N) + &
!                     MCON_XVEC(2,N) * U_XNEG(UM,N) * HMULT_2(UM,N)

!  This is close, BUT..........
              SHOM4 = LCON_XVEC(1,N) * U_XNEG(UM,N) * HMULT_2(UM,N) + &
                     MCON_XVEC(1,N) * U_XNEG(UM,N) * HMULT_1(UM,N) + &
                     LCON_XVEC(2,N) * U_XPOS(UM,N) * HMULT_1(UM,N) + &
                     MCON_XVEC(2,N) * U_XPOS(UM,N) * HMULT_2(UM,N)

              LAYERSOURCE(UM) = SHOM4

!if ( FOURIER.eq.0)write(*,*)SHOM2,SHOM4
...
!if ( FOURIER.eq.0)write(*,*)FOURIER,CONSSCAT(1),LAYERSOURCE(1) 
...
!  Only present if not beyond the cut-off layer
        ...
        IF ( DO_SOLAR_SOURCES .and. N .LE. BEAM_CUTOFF(IB) ) THEN
...
!  Rob Fix 5/24/17 for 2p4a, reintroduce classical PI methods (Version 2.0)
!     Multipliers PMULT_UD, PMULT_UU; SPAR = Greens function addition

          IF ( DO_CLASSICAL_PI ) THEN
            DO LUM = 1, N_PPSTREAMS
              UM = PPSTREAM_MASK(LUM,IB)
              SFOR2 =  EMULT_UP(LUM,N,IB) * U_WPOS2(UM,N)
...
SUBROUTINE TWOSTREAM_DNUSER_INTENSITY &
   ( MAXLAYERS, MAXBEAMS, MAX_USER_STREAMS,                        & ! Dimensions
     DO_CLASSICAL_PI, DO_TRIDIAG, DO_USER_OBSGEOMS,                & ! inputs !@@ 2p1, Control RT
     DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_2S_LEVELOUT,      & ! inputs !@@ 2p2
     FOURIER, IPARTIC, NLAYERS, N_USER_STREAMS, TAYLOR_ORDER, PI4, & ! inputs !@@ 2p3 Greens
...
!  Solution constants of integration, and related quantities (added 3/28/18)
...
!  Conservative scattering solutions. Introduced 3/28/18
      ...
      REAL(kind=dp), INTENT(IN)  :: CSPPHOMDN(2,MAX_USER_STREAMS,MAXLAYERS)
...
!  Rob Fix 3/28/18 for 2p4a, Explicitly calculate Conservative scattering
        ...
        IF ( CONSSCAT(N) ) THEN
          DO LUM = 1, N_PPSTREAMS
            UM = PPSTREAM_MASK(LUM,IB)
            SHOM = LCON(N) * CSPPHOMDN(1,UM,N) + MCON(N) * CSPPHOMDN(2,UM,N) 
            LAYERSOURCE(UM) = SHOM
          ENDDO
        ELSE
          IF ( DO_TRIDIAG ) THEN
            DO LUM = 1, N_PPSTREAMS
              UM = PPSTREAM_MASK(LUM,IB)
              SHOM = LCON_XVEC(1,N) * U_XNEG(UM,N) * HMULT_1(UM,N) + &
                     MCON_XVEC(1,N) * U_XNEG(UM,N) * HMULT_2(UM,N) + &
                     LCON_XVEC(2,N) * U_XPOS(UM,N) * HMULT_2(UM,N) + &
                     MCON_XVEC(2,N) * U_XPOS(UM,N) * HMULT_1(UM,N)
              LAYERSOURCE(UM) = SHOM
...
!  Rob Fix 5/24/17 for 2p4a, reintroduce classical PI methods (Version 2.0)
!     Multipliers PMULT_DU, PMULT_DD; SPAR = Greens function addition

          IF ( DO_CLASSICAL_PI ) THEN
            DO LUM = 1, N_PPSTREAMS
              UM = PPSTREAM_MASK(LUM,IB)
              SFOR2 =  EMULT_DN(LUM,N,IB) * U_WNEG2(UM,N)
              LAYERSOURCE(UM) = LAYERSOURCE(UM) + SFOR2
            ENDDO
          ELSE
            DO LUM = 1, N_PPSTREAMS
...
!  Downward recursion for source function. 
...
SUBROUTINE TWOSTREAM_FLUXES &
            ( MAXBEAMS, MAXLAYERS, DO_UPWELLING, DO_DNWELLING,              & ! Dims/Flags
              DO_2S_LEVELOUT, DO_DBEAM, DO_EDDINGTON_FLUX,                  & ! Flags
              IBEAM, NLAYERS, PI4, STREAM_VALUE, FLUXFAC,                   & ! Input Control
...
!  6/29/24. Eddington Flux solution needs a little care with Flux Factors
...
!  Local variables
      ...
      INTEGER       :: N, N1
      REAL(kind=dp) :: LOCAL_FLUXMULT, PI, PI2, SHOM, SPAR, &
                       QUADINTENS_TOA, QUADINTENS_BOA, QUADINTENS, DMEAN, DFLUX

!  upwelling Flux at TOA


      PI2 = half * PI4 ; PI = half * PI2
      IF ( DO_EDDINGTON_FLUX ) then
        LOCAL_FLUXMULT = FLUXMULT / PI
      ELSE
        LOCAL_FLUXMULT = FLUXMULT
      ENDIF
      ...
      if ( DO_UPWELLING ) THEN
         ...
         ENDIF
         SPAR = WUPPER(2,N)
         QUADINTENS_TOA = LOCAL_FLUXMULT * ( SPAR + SHOM )
         FLUXES_TOA(IBEAM,1) = half * QUADINTENS_TOA
         FLUXES_TOA(IBEAM,2) = PI2 * STREAM_VALUE * QUADINTENS_TOA
      endif

!  Upwelling Flux at other levels

      if ( DO_2S_LEVELOUT )  THEN
         FLUXLEVEL_UP(IBEAM,0,1:2) = FLUXES_TOA(IBEAM,1:2)
         DO N = 1, NLAYERS
            IF ( CONSSCAT(N) ) THEN
               SHOM = - LCON(N) * TGM(N) + MCON(N) * ( ONE + TGM(N) ) !  omega=1
            ELSE
               SHOM = LCON_XVEC(2,N) * EIGENTRANS(N) + MCON_XVEC(2,N)
            ENDIF
            SPAR = WLOWER(2,N)
            QUADINTENS = LOCAL_FLUXMULT * ( SPAR + SHOM )
            FLUXLEVEL_UP(IBEAM,N,1) = half * QUADINTENS
            FLUXLEVEL_UP(IBEAM,N,2) = PI2 * STREAM_VALUE * QUADINTENS
         ENDDO
      endif
...
!  Rob Fix 5/24/17 for 2p4a, Develop handles for Conservative scattering
      ...
      if ( DO_DNWELLING ) THEN
         N = NLAYERS
         IF ( CONSSCAT(N) ) THEN
           SHOM = MCON(N) * tgm(n) + LCON(n) * ( one - tgm(n) )  !  omega=1
!write(0,*) MCON(N) * tgm(n), LCON(n) * tgm(n), LCON(n), SHOM
         ELSE
           SHOM = LCON_XVEC(1,N) * EIGENTRANS(N) + MCON_XVEC(1,N)
         ENDIF
         SPAR = WLOWER(1,N)
         QUADINTENS_BOA = LOCAL_FLUXMULT * ( SPAR + SHOM )
         FLUXES_BOA(IBEAM,1) = half * QUADINTENS_BOA             !actinic flux (diffuse)
...
!  Downwelling Flux at other levels

      if ( DO_2S_LEVELOUT )  THEN
         FLUXLEVEL_DN(IBEAM,NLAYERS,1:2) = FLUXES_BOA(IBEAM,1:2)
         DO N = NLAYERS-1,0,-1
            N1 = N + 1
            IF ( CONSSCAT(N1) ) THEN
               SHOM = LCON(N1) !  omega=1
            ELSE
               SHOM = LCON_XVEC(1,N1) + MCON_XVEC(1,N1) * EIGENTRANS(N1)
            ENDIF
            SPAR = WUPPER(1,N1)
            QUADINTENS = LOCAL_FLUXMULT * ( SPAR + SHOM )
            FLUXLEVEL_DN(IBEAM,N,1) = half * QUADINTENS
            FLUXLEVEL_DN(IBEAM,N,2) = PI2 * STREAM_VALUE * QUADINTENS
            IF ( DO_DBEAM ) THEN
               DMEAN = FLUXFAC * INITIAL_TRANS(N1,IBEAM)  / PI4
               DFLUX = FLUXFAC * INITIAL_TRANS(N1,IBEAM) * X0(IBEAM)
               FLUXLEVEL_DN(IBEAM,N,1) = FLUXLEVEL_DN(IBEAM,N,1) + DMEAN         !actinic flux (difffuse + direct)
               FLUXLEVEL_DN(IBEAM,N,2) = FLUXLEVEL_DN(IBEAM,N,2) + DFLUX         !regular flux (difffuse + direct)
            endif
         ENDDO
      endif
