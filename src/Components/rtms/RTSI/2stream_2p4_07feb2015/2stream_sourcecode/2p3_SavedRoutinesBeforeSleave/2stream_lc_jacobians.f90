...
SUBROUTINE TWOSTREAM_UPUSER_COLUMNWF & 
       ( MAXLAYERS, MAXBEAMS, MAX_USER_STREAMS, MAX_ATMOSWFS,            & ! Dimensions
         ...
         N_USER_STREAMS, N_COLUMN_WFS, PI4,                              & ! input
         ...
         L_U_XNEG, LC_U_WPOS2, HMULT_1, HMULT_2, EMULT_UP, CUMSOURCE_UP, & ! input
...
!            !@@ 2p2, Zero the new "All-level" output (Already Pre-zeroed in LCS FOURIER MASTER)
      ...
      IF ( LUOGSS ) THEN
        DO Q = 1, N_COLUMN_WFS
          COLUMNWF_F_UP(LUM,IB,Q)  = 0.0d0
        ENDDO
      ELSE
        DO UM = 1, N_USER_STREAMS
          DO Q = 1, N_COLUMN_WFS
            COLUMNWF_F_UP(UM,IB,Q)  = 0.0d0
          ENDDO
        ENDDO
      ENDIF
      ...
      IF ( DO_INCLUDE_SURFACE )THEN
         ...
         DO Q = 1, N_COLUMN_WFS
            ...
            ELSE
               ...
               ELSE
                  L_BOA_MSSOURCE(1:N_USER_STREAMS,Q) = KMULT
               ENDIF
            ENDIF
         ENDDO
      ENDIF
...
!         !@@ 2p2, Set All-level output at surface
      ...
      IF ( LUOGSS ) THEN
        DO Q = 1, N_COLUMN_WFS
          ...
          if ( DO_2S_LEVELOUT ) COLJACLEVEL_F_UP(LUM,IB,NLAYERS,Q) = FLUX_MULTIPLIER * L_CUMULSOURCE(IB,Q)
        ENDDO
      ELSE
        DO UM = 1, N_USER_STREAMS
          DO Q = 1, N_COLUMN_WFS
            L_CUMULSOURCE(UM,Q) = L_BOA_MSSOURCE(UM,Q) 
            if ( DO_2S_LEVELOUT ) COLJACLEVEL_F_UP(UM,IB,NLAYERS,Q) = FLUX_MULTIPLIER * L_CUMULSOURCE(UM,Q)
          ENDDO
        ENDDO
      ENDIF
...
!         !@@ 2p2, Set All-level outputs, 7/17/13
        ...
        IF ( LUOGSS ) THEN
           DO Q = 1, N_COLUMN_WFS
              ...
              if (DO_2S_LEVELOUT)COLJACLEVEL_F_UP(LUM,IB,N1,Q) = FLUX_MULTIPLIER * L_CUMULSOURCE(IB,Q)
           ENDDO
        ELSE
          DO UM = 1, N_USER_STREAMS
            DO Q = 1, N_COLUMN_WFS
              ...
              if (DO_2S_LEVELOUT)COLJACLEVEL_F_UP(UM,IB,N1,Q) = FLUX_MULTIPLIER * L_CUMULSOURCE(UM,Q)
            ENDDO
          ENDDO
        ENDIF
...
!         !@@ 2p1, New OBSGEOM option 12/21/12
      ...
      IF ( LUOGSS ) THEN
        DO Q = 1, N_COLUMN_WFS
          COLUMNWF_F_UP(LUM,IB,Q) = FLUX_MULTIPLIER * L_CUMULSOURCE(IB,Q)
        ENDDO
      ELSE
        DO UM = 1, N_USER_STREAMS
          DO Q = 1, N_COLUMN_WFS
            COLUMNWF_F_UP(UM,IB,Q) = FLUX_MULTIPLIER * L_CUMULSOURCE(UM,Q)
          ENDDO
        ENDDO
      ENDIF
...
!            !@@ 2p2, Zero the new "All-level" output (Already Pre-zeroed in LCS FOURIER MASTER)
      ...
      IF ( LUOGSS ) THEN
        DO Q = 1, N_COLUMN_WFS
          COLUMNWF_F_DN(LUM,IB,Q)               = 0.0d0
        ENDDO
      ELSE
        DO UM = 1, N_USER_STREAMS
          DO Q = 1, N_COLUMN_WFS
            COLUMNWF_F_DN(UM,IB,Q)               = 0.0d0
          ENDDO
        ENDDO
      ENDIF
...
!         !@@ 2p2, Set All-level outputs, 7/17/13
        ...
        IF ( LUOGSS ) THEN
          DO Q = 1, N_COLUMN_WFS
            ...
            if (DO_2S_LEVELOUT)COLJACLEVEL_F_DN(LUM,IB,N,Q) = FLUX_MULTIPLIER * L_CUMULSOURCE(IB,Q)
          ENDDO
        ELSE
          DO UM = 1, N_USER_STREAMS
            DO Q = 1, N_COLUMN_WFS
              ...
              if (DO_2S_LEVELOUT)COLJACLEVEL_F_DN(UM,IB,N,Q) = FLUX_MULTIPLIER * L_CUMULSOURCE(UM,Q)  
            ENDDO
          ENDDO
        ENDIF
...
!         !@@ 2p1, New OBSGEOM option 12/21/12
      ...
      IF ( LUOGSS ) THEN
        DO Q = 1, N_COLUMN_WFS
          COLUMNWF_F_DN(LUM,IB,Q) = FLUX_MULTIPLIER * L_CUMULSOURCE(IB,Q)
        ENDDO
      ELSE
        DO UM = 1, N_USER_STREAMS
          DO Q = 1, N_COLUMN_WFS
            COLUMNWF_F_DN(UM,IB,Q) = FLUX_MULTIPLIER * L_CUMULSOURCE(UM,Q)
          ENDDO
        ENDDO
      ENDIF
!  Finish
...
!     ! @@ Rob Spurr, 05 November 2013, Version 2.3 --> Flux Output
      ...
      REAL(kind=dp), INTENT(INOUT) :: COLJACFLUXES_TOA(MAXBEAMS,2,MAX_ATMOSWFS,MAXTHREADS)
      REAL(kind=dp), INTENT(INOUT) :: COLJACFLUXES_BOA(MAXBEAMS,2,MAX_ATMOSWFS,MAXTHREADS)
...
!  upwelling Flux at TOA
      ...
      if ( DO_UPWELLING ) THEN
         ...
         DO Q = 1, N_COLUMN_WFS
            ...
            COLJACFLUXES_TOA(IBEAM,1,Q,THREAD) = 0.5d0 * LC_QUADINTENS_TOA
            COLJACFLUXES_TOA(IBEAM,2,Q,THREAD) = SPI2  * LC_QUADINTENS_TOA
         ENDDO
      endif
...
!  Downwelling Flux at BOA
      ...
      if ( DO_DNWELLING ) THEN
         ...
         DO Q = 1, N_COLUMN_WFS
            ...
            COLJACFLUXES_BOA(IBEAM,1,Q,THREAD) = 0.5d0 * LC_QUADINTENS_BOA
            COLJACFLUXES_BOA(IBEAM,2,Q,THREAD) = SPI2  * LC_QUADINTENS_BOA
         ENDDO
      endif
...
SUBROUTINE TWOSTREAM_LCS_CONVERGE &
      ( MAXBEAMS, MAX_USER_STREAMS, MAX_USER_RELAZMS, MAX_GEOMETRIES,                & ! Dimensions
        ...
        NLAYERS, THREAD, IBEAM, FOURIER, N_COLUMN_WFS, N_SURFACE_WFS,                & ! Inputs     ! @@ 2p2
...
!  Fourier component and thread, nlayers (2p2, added)

      INTEGER, INTENT(IN)  :: FOURIER, NLAYERS, THREAD, IBEAM
...
!  ------
      ...
      REAL(kind=dp), INTENT(INOUT) :: COLUMNWF_TOA(MAX_GEOMETRIES,MAX_ATMOSWFS,MAXTHREADS)
      REAL(kind=dp), INTENT(INOUT) :: COLUMNWF_BOA(MAX_GEOMETRIES,MAX_ATMOSWFS,MAXTHREADS)
...
!     ! @@ Rob Spurr, 17 July 2013, Version 2.2 --> Optional Output at ALL LEVELS
      ...
      REAL(kind=dp), INTENT(INOUT) :: COLJACLEVEL_UP (MAX_GEOMETRIES,0:MAXLAYERS,MAX_ATMOSWFS,MAXTHREADS)
      REAL(kind=dp), INTENT(INOUT) :: COLJACLEVEL_DN (MAX_GEOMETRIES,0:MAXLAYERS,MAX_ATMOSWFS,MAXTHREADS)
        ...
        DO Q = 1, N_COLUMN_WFS
           DO I = 1, N_USER_STREAMS
              DO UA = 1, N_USER_RELAZMS
                 V = UMOFF(IBEAM,I) + UA
                 IF ( DO_UPWELLING ) THEN
                    COLUMNWF_TOA(V,Q,THREAD) = COLUMNWF_F_UP(I,IBEAM,Q)
                    IF ( DO_2S_LEVELOUT ) COLJACLEVEL_UP(V,0:NLAYERS,Q,THREAD) = COLJACLEVEL_F_UP(I,IBEAM,0:NLAYERS,Q)
                 ENDIF
                 IF ( DO_DNWELLING ) THEN
                    COLUMNWF_BOA(V,Q,THREAD) = COLUMNWF_F_DN(I,IBEAM,Q)
                    IF ( DO_2S_LEVELOUT ) COLJACLEVEL_DN(V,0:NLAYERS,Q,THREAD) = COLJACLEVEL_F_DN(I,IBEAM,0:NLAYERS,Q)
                 ENDIF
              ENDDO
           ENDDO
        ENDDO
         ...
         DO UA = 1, N_USER_RELAZMS
          DO I = 1, N_USER_STREAMS
           ...
           IF ( DO_UPWELLING ) THEN
            ...
            IF ( DO_2S_LEVELOUT ) COLJACLEVEL_UP(V,0:NLAYERS,Q,THREAD) = &
                   COLJACLEVEL_UP(V,0:NLAYERS,Q,THREAD) + AZMFAC(I,IBEAM,UA) * COLJACLEVEL_F_UP(I,IBEAM,0:NLAYERS,Q)
           ENDIF
           IF ( DO_DNWELLING ) THEN
            TOLD = COLUMNWF_BOA(V,Q,THREAD)
            TAZM = AZMFAC(I,IBEAM,UA)*COLUMNWF_F_DN(I,IBEAM,Q)
            COLUMNWF_BOA(V,Q,THREAD) = TOLD + TAZM
            IF ( DO_2S_LEVELOUT ) COLJACLEVEL_DN(V,0:NLAYERS,Q,THREAD) = &
                   COLJACLEVEL_DN(V,0:NLAYERS,Q,THREAD) + AZMFAC(I,IBEAM,UA) * COLJACLEVEL_F_DN(I,IBEAM,0:NLAYERS,Q)
           ENDIF
          ENDDO
         ENDDO
       ENDDO
...
SUBROUTINE TWOSTREAM_LCS_CONVERGE_OBSGEO &
      ( MAXBEAMS, MAX_USER_STREAMS, MAX_USER_RELAZMS, MAX_GEOMETRIES,                & ! Dimensions
        MAXTHREADS, MAXLAYERS, MAX_ATMOSWFS, MAX_SURFACEWFS,                         & ! Dimensions ! @@ 2p2
        DO_UPWELLING, DO_DNWELLING, DO_2S_LEVELOUT, DO_BRDF_SURFACE, DO_SURFACE_WFS, & ! Inputs     ! @@ 2p2
        NLAYERS, THREAD, IBEAM, FOURIER, N_COLUMN_WFS, N_SURFACE_WFS, AZMFAC,        & ! Inputs     ! @@ 2p2
        COLUMNWF_F_UP,   COLUMNWF_F_DN,  COLJACLEVEL_F_UP,  COLJACLEVEL_F_DN,        & ! Inputs     ! @@ 2p2
        SURFACEWF_F_UP,  SURFACEWF_F_DN, SURFJACLEVEL_F_UP, SURFJACLEVEL_F_DN,       & ! Inputs     ! @@ 2p2
        COLUMNWF_TOA,    COLUMNWF_BOA,   COLJACLEVEL_UP,    COLJACLEVEL_DN,          & ! In/Out     ! @@ 2p2
        SURFACEWF_TOA,   SURFACEWF_BOA,  SURFJACLEVEL_UP,   SURFJACLEVEL_DN  )         ! In/Out     ! @@ 2p2
...
!  Diffuse field at all output angles
        ...
        DO Q = 1, N_COLUMN_WFS
           IF ( DO_UPWELLING ) THEN
              COLUMNWF_TOA(IBEAM,Q,THREAD) = COLUMNWF_F_UP(LUM,IBEAM,Q)
              IF ( DO_2S_LEVELOUT ) COLJACLEVEL_UP(IBEAM,0:NLAYERS,Q,THREAD) = COLJACLEVEL_F_UP(LUM,IBEAM,0:NLAYERS,Q)
           ENDIF
           IF ( DO_DNWELLING ) THEN
              COLUMNWF_BOA(IBEAM,Q,THREAD) = COLUMNWF_F_DN(LUM,IBEAM,Q)
              IF ( DO_2S_LEVELOUT ) COLJACLEVEL_DN(IBEAM,0:NLAYERS,Q,THREAD) = COLJACLEVEL_F_DN(LUM,IBEAM,0:NLAYERS,Q)
           ENDIF
        ENDDO
...
!  --------------------------------------
       ...
       DO Q = 1, N_COLUMN_WFS
          IF ( DO_UPWELLING ) THEN
             TOLD = COLUMNWF_TOA(IBEAM,Q,THREAD)
             COLUMNWF_TOA(IBEAM,Q,THREAD) = TOLD + AZFAC * COLUMNWF_F_UP(LUM,IBEAM,Q)
             IF ( DO_2S_LEVELOUT ) COLJACLEVEL_UP(IBEAM,0:NLAYERS,Q,THREAD) = &
                      COLJACLEVEL_UP(IBEAM,0:NLAYERS,Q,THREAD) + AZFAC * COLJACLEVEL_F_UP(LUM,IBEAM,0:NLAYERS,Q)
          ENDIF
          IF ( DO_DNWELLING ) THEN
             TOLD = COLUMNWF_BOA(IBEAM,Q,THREAD)
             COLUMNWF_BOA(IBEAM,Q,THREAD) = TOLD + AZFAC * COLUMNWF_F_DN(LUM,IBEAM,Q)
             IF ( DO_2S_LEVELOUT ) COLJACLEVEL_DN(IBEAM,0:NLAYERS,Q,THREAD) = &
                      COLJACLEVEL_DN(IBEAM,0:NLAYERS,Q,THREAD) + AZFAC * COLJACLEVEL_F_DN(LUM,IBEAM,0:NLAYERS,Q)
           ENDIF
       ENDDO
